...
class GUI:
    ...
    def robots_command(self):
        path = self.path
        last_orientation = self.robot_position[int(
            self.controlled_robot_id)-1, 2]
        task_time = 2000
        velocity = 35/500
        task_id = 0
        ...
        for i in range(1, len(path)):
            ...
            if(new_orientation == 0):
                ...
                print(
                    f"task {task_id}, distance {distance}, rotattion angle {new_orientation}, time {task_time}")
                self.xbee.send_msg_unicast(
                ...
                if(new_orientation > 0):
                    ...
                    task_id = 5
                    frame.send_msg(task_id=task_id, distance=0, task_time=int(
                        task_time), arc_radius=0, rotation_angle=int(abs(new_orientation)))
        ...
        frame.send_msg(task_id=1, distance=int(distance), task_time=int(
            task_time), arc_radius=10, rotation_angle=int(new_orientation))
        print(
