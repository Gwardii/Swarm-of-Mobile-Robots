...
class RPI_Communication_Server:
    def __init__(self, host="localhost", port=9999):
        ...
        self._host = host
        self._port = port
        self._rpi_server_socket = socket.socket(
            socket.AF_INET, socket.SOCK_STREAM)  # socket - IPv4 ,TCP protocol
        self._rpi_server_socket.setsockopt(
            socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        self._rpi_server_socket.bind((self._host, self._port))

        # start separated thread for server:
        self._rpi_communication_thread = threading.Thread(
            target=self._server_listen_)
