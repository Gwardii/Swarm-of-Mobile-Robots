...
This page is about the main computer which is a brain of a whole swarm. It computes paths, decides on the movements of the robots, and communicates with them. Also, it runs the main GUI and receives data from RPI. Below are descriptions of functions and programs that run on the main computer.

<br/>

### application.py

Main script which starts all the others. After starting the application initialization starts: the application will try to communicate with RPI and receive JSON from it. After 10 [s] without receiving data, it will be terminated and GUI will start to generate. 
The only setting you need to do is to provide IP, PORT of RPI, number of used robots and XBEE mode at the beginning of the program:
```
# --------------USER SETTINGS---------------
...
In order to provide a more orderly and readable code state machine was implemented. All states and possible transitions are shown on the diagram below:

<img src="/Readme_img/main_state_machine_2.png" align="center" style = "width: 80%"  />

All states are implemented as separate classes inherited from a state base class. In this way, all classes have a similar structure and it's easy to figure out what is the task of each state. In addition, a structure like this allows for the easy and quick adding new states. It is important to mention that a state also can be a separate state machine containing some smaller states. In this project, the state Initialization was implemented as another state machine so if you want to implement something like this refer to the class Initialization in states.py. 
...
Later add your state to the AllStates and the AllTransitions classes, which contain names of all possible states and transitions. At the end add your state to state and transition initialization. In order to do that go to application.py and in the _states_initialization and _transition_initialization initialize a new class like classes already added.
...
GUI allows controlling each robot by controlling buttons and also by setting target coordinates. In addition, the app provides a live camera view and information about communication status. The simple terminal enables updating the displayed map with the "update" command. It also can be used to display messages to help debug applications. 

### MAP GENERATION

The displayed map is created based on JSON files received from RPI, and information about all possible obstacles is read from the database. The application receives three JSON files: area.json, obstacles.json and robot.json. The area.json contains the coordinates of all working area corners. Obstacles.json contains coordinates (x, y, rotation) and the id of detected obstacles.  Robots.json contains coordinates (x,y, rotation) and the id of detected robots. In the next step, the map is rasterized because path generation is based on a diffusion algorithm. The map is divided into cells with a given dimension (dimension can be set in application settings). Later each cell's weight is calculated and saved to a 2D array. These values are later used for path computation and visualisation of cells state (gradient informing about the distance to obstacles) 

### PATH COMPUTING
The path planning algorithm is divided into 3 steps:
#### Pre-processing
Once a raster map is obtained, its cells are characterised by their distance from the nearest obstacle or workspace boundary. Next, for each of the robots in this map, available cells are determined based on the radius of the robot. If the robot does not have a circular shape it should be written in a circle and then represented by it. The first step should be processed at the beginning or whenever the map changes.
#### Solver
It is an implementation of a diffusion algorithm for pathfinding. Before moving on, naively check if a path can be traced as a single line between a target and an actual location. If so, return the line as the path and do not include the robot in the diffusion path search. For each robot, an artificial potential field is created. The field has the highest value in the target cell and its values propagate from the cell onto another cell like a wave - assign neighbouring cells lower values and repeat for not assigned cells yet. Next, the path is determined as a chain of cells where the first cell is an actual position of a robot and each successive cell is a neighbour with the highest value.
#### Post-processing
The obtained paths are lists of particular cells, however, the most optimal way to store and transmit paths is to describe them as a polygonal chain. For this purpose, a simple recursive minimalization of a route is implemented.
