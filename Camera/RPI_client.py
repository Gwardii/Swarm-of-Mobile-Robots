...
class RPI_Communication_Client():
    ...
    def _send_message(self, data):
        ...
        try:
            self.message_sended = False
            self.rpi_client_socket = socket.socket(
                socket.AF_INET, socket.SOCK_STREAM)
            self.rpi_client_socket.settimeout(0.01)
            ...
            # close socket if can not establish connection:
            self.rpi_client_socket.close()
            return
    ...
    def _client_listen(self):
        ...
        print(
            f"new message lenght: {self._buffer[:self.HEADER_SIZE].decode('utf-8')}")
